...
contract WorldEventProcessor60523 is DefaultWorldEventProcessor {
    ...
    function eventInfo(uint256 /*_actor*/) external virtual view override returns (string memory) {
        //村民为你制作了简陋的陶土药钵。
        return "\xE6\x9D\x91\xE6\xB0\x91\xE4\xB8\xBA\xE4\xBD\xA0\xE5\x88\xB6\xE4\xBD\x9C\xE4\xBA\x86\xE7\xAE\x80\xE9\x99\x8B\xE7\x9A\x84\xE9\x99\xB6\xE5\x9C\x9F\xE8\x8D\xAF\xE9\x92\xB5\xE3\x80\x82";
    }
    ...
    function checkOccurrence(uint256 _actor, uint256 /*_age*/) external virtual view override returns (bool) {
        ...
        (mt , st) = worldRoute.actors().actor(_actor);
        ...
        //行动力要求
        if(IActorBehaviorAttributes(worldRoute.modules(DahuangConstants.WORLD_MODULE_BEHAVIOR_ATTRIBUTES)).attributesScores(DahuangConstants.ATTR_ACT, _actor) < 5) {
            //require(false, "act point is low");
            return false;
        }

        //资源要求
        if(IWorldFungible(worldRoute.modules(DahuangConstants.WORLD_MODULE_GOLD)).balanceOfActor(_actor) < 1e18) {
            //require(false, "gold is insufficient");
            return false;
        }
        if(IWorldFungible(worldRoute.modules(DahuangConstants.WORLD_MODULE_WOOD)).balanceOfActor(_actor) < 2e18) {
            //require(false, "wood is insufficient");
            return false;
        }
        if(IWorldFungible(worldRoute.modules(DahuangConstants.WORLD_MODULE_FABRIC)).balanceOfActor(_actor) < 1e18)
            return false;
        if(IWorldFungible(worldRoute.modules(DahuangConstants.WORLD_MODULE_HERB)).balanceOfActor(_actor) < 10e18)
            return false;
