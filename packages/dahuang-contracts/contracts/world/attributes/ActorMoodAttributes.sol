...
contract ActorMoodAttributes is IActorAttributes, WorldConfigurable {
    ...
    function _tokenSVG(uint256 _actor, uint256 _startY, uint256 _lineHeight) internal view returns (string memory, uint256 _endY) {
        ...
        if(characterPointsInitiated[_actor]) {
            ...
            _endY += _lineHeight;
            string memory svg1 = string(abi.encodePacked('<text x="20" y="', Strings.toString(_endY), '" class="base">', attributeLabels[DahuangConstants.ATTR_XIQ - DahuangConstants.ATTR_BASE_MOOD], "=", Strings.toString(attributesScores[DahuangConstants.ATTR_XIQ][_actor]), '</text>'));
    ...
    function _tokenJSON(uint256 _actor) internal view returns (string memory) {
        string memory json = '';
        json = string(abi.encodePacked('{"XIQ": ', Strings.toString(attributesScores[DahuangConstants.ATTR_XIQ][_actor]), '}'));
        return json;
    }
    ...
    function moduleID() external override pure returns (uint256) { return DahuangConstants.WORLD_MODULE_MOOD_ATTRIBUTES; }
    ...
    {        
        ...
        uint256 _maxPointBuy = talents.actorAttributePointBuy(_actor, DahuangConstants.WORLD_MODULE_MOOD_ATTRIBUTES);
        attributesScores[DahuangConstants.ATTR_XIQ][_actor] = 100;
        if(_maxPointBuy > 0)
            attributesScores[DahuangConstants.ATTR_XIQ][_actor] = _maxPointBuy;

        characterPointsInitiated[_actor] = true;

        uint256[] memory atts = new uint256[](2);
        atts[0] = DahuangConstants.ATTR_XIQ;
        atts[1] = attributesScores[DahuangConstants.ATTR_XIQ][_actor];
        ...
        onlyPointsInitiated(_actor)
    {
        ...
        for(uint256 i=0; i<_attributes.length; i+=2) {
            if(_attributes[i] == DahuangConstants.ATTR_XIQ) {
                attributesScores[DahuangConstants.ATTR_XIQ][_actor] = _attributes[i+1];
                updated = true;
                break;
            }
        }
    ...
    function applyModified(uint256 _actor, int[] memory _modifiers) external view override returns (uint256[] memory, bool) {
        ...
        uint256 xiq = attributesScores[DahuangConstants.ATTR_XIQ][_actor];
        for(uint256 i=0; i<_modifiers.length; i+=2) {
            if(_modifiers[i] == int(DahuangConstants.ATTR_XIQ)) {
