...
describe('主动事件角色行为测试', () => {
    ...
    describe('创建新区域和基础资源采集与兑换', () => {
        ...
        it(`角色在当前所在区域采集资源`, async ()=>{
            ...
            while((await golds.balanceOfActor(testActor)).lt(BigInt(1e18))) {
        ...
        it('配置60514经手人角色', async () => {
            ...
            await actors.connect(deployer).approve(evt60514.address, evt60515Operator);
            await evt60514.initOperator(evt60515Operator);
            expect(await evt60514.eventOperator()).to.eq(evt60515Operator);
            expect(await actors.ownerOf(evt60515Operator)).to.eq(evt60514.address);
        });
        ...
        it(`给60514经手人充值道理`, async ()=>{
            await assetDaoli.connect(taiyiDAO).claim(actorPanGu, actorPanGu, BigInt(1000e18));
            await assetDaoli.connect(taiyiDAO).transferActor(actorPanGu, evt60515Operator, BigInt(1000e18));
        });

        it(`60514兑换合约执行`, async ()=>{
            expect(await evt60514.checkOccurrence(testActor, 0)).to.eq(true);
            let assetId = await golds.moduleID();
            let amount = await golds.balanceOfActor(testActor);
