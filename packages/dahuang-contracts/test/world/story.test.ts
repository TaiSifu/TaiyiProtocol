...
import { DahuangContractName, deployDahuangWorld, deployGlobalStoryTimeline } from '../../utils';
...
describe('剧情测试', () => {
    ...
    describe('全局剧情70001', () => {
        ...
        let storyTimeline: ShejiTu;
        let storyActor: BigNumber;
        let wux: BigNumber;
        before(reset);

        it(`部署全局剧情时间线`, async ()=>{
            await makeMoney(deployer.address, BigInt(100e18));

            let timelinePkg = await deployGlobalStoryTimeline(worldContractRoute.connect(taiyiDAO), "大荒全局剧情", "所在时间线：大荒",
                BigInt(225), actors, actorLocations, zones, worldEvents, talents, trigrams, worldRandom, assetDaoli, worldYemings,
                baseAttributes, charmAttributes, coreAttributes, moodAttributes, behaviorAttributes, taiyiDAO, deployer);
            storyTimeline = ShejiTu__factory.connect((timelinePkg[0] as ShejiTuProxy).address, deployer);

            await storyTimeline.setStartZone(startZone);
            await storyTimeline.addAgeEvent(0, 10001, 1);
        });
        ...
        it(`通过采集资源激活剧情70003`, async ()=>{
            wux = await coreAttributes.attributesScores(await dahuangConstants.ATTR_WUX(), storyActor);
        ...
        it(`剧情70003检查`, async ()=>{
            ...
            expect(await worldEvents.ages(storyActor)).to.eq(0);
            expect(await coreAttributes.attributesScores(await dahuangConstants.ATTR_WUX(), storyActor)).to.eq(wux.add(10));
            let evts = await worldEvents.actorEvent(storyActor, 0);
