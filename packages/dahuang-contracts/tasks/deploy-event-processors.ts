...
task('deploy-event-processors', '部署大荒事件合约')
    .setAction(async (args, { ethers }) => {
        ...
        console.log(`部署事件`);
        let evt10027 = await (await (new WorldEventProcessor10027__factory(deployer)).deploy(worldContractRoute.address)).deployed();
        let evt10027Args = [worldContractRoute.address];
        await (await worldEvents.setEventProcessor(10027, evt10027.address)).wait();
                    
        //save contract address
        addressBook.WorldEventProcessor10027 = evt10027.address;
        ...
        argsBook.WorldEventProcessor10027 = evt10027Args;
        ...
        console.log(`配置时间线3岁`);
        await (await shejiTu.connect(deployer).addAgeEvent(3, 10027, 1)).wait();
        console.log(`配置时间线4岁`);
        await (await shejiTu.connect(deployer).addAgeEvent(4, 10027, 1)).wait();
        console.log(`配置时间线5岁`);
        await (await shejiTu.connect(deployer).addAgeEvent(5, 10027, 200)).wait();
        console.log(`配置时间线6岁`);
        await (await shejiTu.connect(deployer).addAgeEvent(6, 10027, 200)).wait();
        console.log(`配置时间线7岁`);
