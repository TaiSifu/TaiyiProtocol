...
task('deploy-event-processors', '部署大荒事件合约')
    .setAction(async (args, { ethers }) => {
        ...
        console.log(`部署事件`);
        let evt10030 = await (await (new WorldEventProcessor10030__factory(deployer)).deploy(worldContractRoute.address)).deployed();
        let evt10030Args = [worldContractRoute.address];
        await (await worldEvents.setEventProcessor(10030, evt10030.address)).wait();
        let evt10062 = await (await (new WorldEventProcessor10062__factory(deployer)).deploy(worldContractRoute.address)).deployed();
        let evt10062Args = [worldContractRoute.address];
        await (await worldEvents.setEventProcessor(10062, evt10062.address)).wait();
                    
        //save contract address
        addressBook.WorldEventProcessor10030 = evt10030.address;
        addressBook.WorldEventProcessor10062 = evt10062.address;
        ...
        argsBook.WorldEventProcessor10030 = evt10030Args;
        argsBook.WorldEventProcessor10062 = evt10062Args;
