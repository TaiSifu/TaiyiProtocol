...
(0, config_1.task)('show-actor-history', '显示角色经历')
    ...
    if ((yield actors.mintTime(actor)).eq(0)) {
        console.log(`角色#${actor}不存在。`);
        return;
    }
    let currentAge = (yield events.ages(actor)).toNumber();
    let name = (yield names.actorName(actor))._name;
    name = (name == "" ? "无名氏" : name);
    console.log(`${name}(角色#${actor})的成长经历：`);
    for (var a = 0; a <= currentAge; a++) {
        console.log(`${a == 0 ? "出生" : (a + "岁")}：`);
        let evts = yield events.actorEvent(actor, a);
        for (var e = 0; e < evts.length; e++) {
            let eventId = evts[e];
            let eventInfo = yield events.eventInfo(eventId, actor);
