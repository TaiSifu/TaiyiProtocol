import { Account, Delegate, Proposal, Governance, Vote, Event } from '../types/schema';
...
export function getEventEntity(): Event {
    let evt = Event.load('EVENT');

    if (evt == null) {
        evt = new Event('EVENT');
        evt.evtId = BIGINT_ZERO;
        evt.age = BIGINT_ZERO;
    }
