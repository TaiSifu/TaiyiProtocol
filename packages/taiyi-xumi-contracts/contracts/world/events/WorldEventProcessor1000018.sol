...
contract WorldEventProcessor1000018 is DefaultWorldEventProcessor {
    ...
    function checkOccurrence(uint256 _actor, uint256 /*_age*/) external view override returns (bool) {
        ...
        //"include": "EVT?[1000001,1020001,1020002]",
        defaultRt = false; //default should be fasle if have "include" conditions
        if(evts.actorEventCount(_actor, 1000001) > 0)
            return true;
        if(evts.actorEventCount(_actor, 1020001) > 0)
            return true;
        if(evts.actorEventCount(_actor, 1020002) > 0)
            return true;
    ...
    // "branch": [
    //     "EVT?[100006,1000009]=>(1000019 on prob 0.5)"
    // ]
