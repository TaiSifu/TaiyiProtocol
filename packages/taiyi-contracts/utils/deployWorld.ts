...
import {
    ...
    TrigramsRender, TrigramsRender__factory,
} from '../typechain';
import { BigNumber, BigNumberish, Contract as EthersContract } from 'ethers';
import { blockNumber, blockTimestamp, deploySifusToken, deploySifusDescriptor, populateDescriptor } from '../test/utils';
...
export const deployTaiyiWorld = async (oneAgeVSecond : number, actRecoverTimeDay: number, zoneResourceGrowTimeDay : number, zoneResourceGrowQuantityScale: number,
    ...
    let actors = await deployActors(operatorDAO.address, timestamp, assetDaoli.address, worldContractRoute, deployer);
    ...
    let shejiTu = await deployShejiTu(sifusToken, worldContractRoute, deployer);
