...
export async function deployEvents(evtMaps:{[index: string]:any}, route: factory.WorldContractRoute, eventsAddress: string, operator: Signer, deployer: Signer) : Promise<any> {
    let addressBook :{[index: string]:any} = {};
    let events = factory.WorldEvents__factory.connect(eventsAddress, operator);
...
export async function initEvents(route: factory.WorldContractRoute, eventsAddress: string, operator: Signer, deployer: Signer) : Promise<any> {
