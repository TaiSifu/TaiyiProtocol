...
describe('角色姓名测试', () => {
    ...
    before(async () => {
        ...
        actorNames = await deployActorNames(worldContractRoute, deployer);
        await worldContractRoute.connect(taiyiDAO).registerModule(await worldConstants.WORLD_MODULE_NAMES(), actorNames.address);
    });
