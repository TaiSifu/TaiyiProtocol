...
import {
    ...
    Actors, WorldItems, SifusToken, SifusDescriptor__factory, WorldYemings, AssetDaoli, NameGenerator,
} from '../../typechain';
import {
    blockNumber,
    blockTimestamp,
...
describe('姓名发生器测试', () => {
    ...
    before(async () => {
        ...
        nameGen = await deployNameGenerator(worldContractRoute, deployer);
    ...
    it('注册性别', async () => {
        await nameGen.connect(taiyiDAO).registerGender(actorPanGu, ["男", "女"]);
    });

    it('注册姓', async () => {
        await nameGen.connect(taiyiDAO).registerFamily(actorPanGu, [
            "李",
            "王",
            "张",
            "刘",
    ...
    it('注册辈分', async () => {
        await nameGen.connect(taiyiDAO).registerMiddle(actorPanGu, [
            "之",
            "亦",
            "其",
            "如",
    ...
    it('注册错误性别的名', async () => {
        await expect(nameGen.connect(taiyiDAO).registerGiven(actorPanGu, "非", ["国","民"])).to.be.revertedWith("gender not exist");
    });

    it('注册男名', async () => {
        await nameGen.connect(taiyiDAO).registerGiven(actorPanGu, "男", [
            "国",
            "民",
            "邦",
            "杰",
    ...
    it('注册女名', async () => {
        await nameGen.connect(taiyiDAO).registerGiven(actorPanGu, "女", [
            "兮",
            "芳",
            "星",
            "清",
