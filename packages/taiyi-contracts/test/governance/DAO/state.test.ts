...
describe('太乙岛提案状态测试', () => {
    ...
    it('无效的提案ID', async () => {
        await makeProposal();
        await expect(gov.state(5)).revertedWith('TaiyiDAO::state: invalid proposal id');
    });

    it('待投票排队期（Pending）', async () => {
        await makeProposal();
        expectState(proposalId, 'Pending');
    });

    it('进入投票期（Active）', async () => {
        await makeProposal();
        expectState(proposalId, 'Pending');
    ...
    it('投票期超时后无投票被否决（Defeated）', async () => {
        await makeProposal();
        // travel to end block
    ...
    it('进入执行队列后，被任何人执行（Executed）', async () => {
        await makeProposal();
        await mineBlock();
